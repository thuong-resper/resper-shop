...
import { getListProductWoman } from 'features/Product/pathApi';
...
const ProductListWoman = () => {
  const dispatch = useDispatch();

  const [value, setValue] = useState(0);
  const [sex] = useState('woman');
  ...
  useEffect(() => {
    ...
    const actionGetListProducts = (params) => dispatch(getListProductWoman(params));
    actionGetListProducts(params);
  }, [category, sex, dispatch]);

  const data = useSelector((state) => state.ListProductWoman.data);
  const loading = useSelector((state) => state.ListProductWoman.loading);
  const error = useSelector((state) => state.ListProductWoman.error);
                ...
                <TabPanel className="tab-panel" value={value} index={index} key={index}>
                  {fetchedCategories.map((c) => (
                    ...
                    >
                      {c.index === value ? (
                        <span>
                          Xem thêm&nbsp;<strong>{`${c.label} nữ`}</strong>
                        </span>
                      ) : null}
                    </Link>
                  ))}
  ...
  return (
    <>
      <div className="watch-type premium-section wfwomen">
...
export default ProductListWoman;
